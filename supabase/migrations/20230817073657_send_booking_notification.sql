...
BEGIN
  ...
  INSERT INTO Notifications (usernames, title, message, value, image, link, icon, color)
    SELECT ARRAY[unnest.investor], 'Apartment Booked', house_name || ' has been booked for ' || nights || ' nights!', (SUM(amount) / total) * NEW.income, house_images[1], 'investment/' || NEW.house_id, 'profit', 'success'
